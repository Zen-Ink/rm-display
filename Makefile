...
.PHONY: help clean cargo fmt-check test clippy check quill-aarch64 quill-rm2 \
	receiver-aarch64 receiver-rm2 receiver-takeover receiver-rm2-takeover \
	run-receiver run-cli
...
help:
	@printf '%s\n' \
		...
		'make quill-rm2             Cross-build the RM2 ARMv7 Quill library' \
		'make receiver-aarch64      Cross-build the Quill receiver' \
		'make receiver-rm2          Cross-build the RM2 ARMv7 receiver' \
		'make receiver-takeover     Build the installable AArch64 takeover archive' \
		'make receiver-rm2-takeover Build the installable RM2 takeover archive' \
...
clean:
	...
	rm -rf dist/rm-display-receiver-aarch64 dist/rm-display-receiver-aarch64.tar.gz \
		dist/rm-display-receiver-armv7 dist/rm-display-receiver-armv7.tar.gz
...
quill-aarch64:
	...
	@test -f "$(QUILL_DIR)/vendor/aarch64/libqsgepaper.so" || { printf 'missing AArch64 qsgepaper library\n'; exit 2; }
...
receiver-aarch64: quill-aarch64
	...
	QUILL_LIB_DIR="$(abspath $(QUILL_DIR))/build/aarch64" \
	...
	install -m 0755 "$(QUILL_DIR)/build/aarch64/libquill.so" "$$output/libquill.so"; \
	install -m 0755 "$(QUILL_DIR)/vendor/aarch64/libqsgepaper.so" "$$output/libqsgepaper.so"; \
	printf 'output: %s\n' "$$output"
...
quill-rm2:
	...
	@test -f "$(SDK_ENV)" || { printf 'missing SDK environment: %s\n' "$(SDK_ENV)"; exit 2; }
	@test -x "$(QUILL_DIR)/build.sh" || { printf 'missing Quill submodule; run git submodule update --init\n'; exit 2; }
	@test -f "$(QUILL_DIR)/vendor/armv7/libqsgepaper.so" || { printf 'missing RM2 qsgepaper library\n'; exit 2; }
	@SDK_ENV="$(abspath $(SDK_ENV))" QUILL_LIBRARY_ONLY=1 "$(QUILL_DIR)/build.sh"

receiver-rm2: quill-rm2
	@unset LD_LIBRARY_PATH; source "$(SDK_ENV)"; \
	test -n "$${SDKTARGETSYSROOT:-}" || { printf 'SDK does not export SDKTARGETSYSROOT\n'; exit 2; }; \
	QUILL_LIB_DIR="$(abspath $(QUILL_DIR))/build/armv7" \
	CARGO_TARGET_ARMV7_UNKNOWN_LINUX_GNUEABIHF_RUSTFLAGS="-C target-cpu=cortex-a7 -C link-arg=--sysroot=$$SDKTARGETSYSROOT -C link-arg=-mcpu=cortex-a7 -C link-arg=-mfpu=neon -C link-arg=-mfloat-abi=hard" \
		cargo build --release --target armv7-unknown-linux-gnueabihf \
		-p rm-display-receiver --features quill
	@output="$$(cargo metadata --no-deps --format-version=1 2>/dev/null | \
		sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')/armv7-unknown-linux-gnueabihf/release"; \
	install -m 0755 "$(QUILL_DIR)/build/armv7/libquill.so" "$$output/libquill.so"; \
	install -m 0755 "$(QUILL_DIR)/vendor/armv7/libqsgepaper.so" "$$output/libqsgepaper.so"; \
	printf 'output: %s\n' "$$output"
...
receiver-rm2-takeover: receiver-rm2
	@output="$$(cargo metadata --no-deps --format-version=1 2>/dev/null | \
		sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')/armv7-unknown-linux-gnueabihf/release"; \
	stage="dist/rm-display-receiver-armv7"; \
	archive="dist/rm-display-receiver-armv7.tar.gz"; \
	rm -rf "$$stage" "$$archive"; \
	install -d "$$stage/scripts"; \
	install -m 0755 "$$output/rm-display-receiver" "$$stage/rm-display-receiver"; \
	install -m 0755 "$$output/libquill.so" "$$stage/libquill.so"; \
	install -m 0755 "$$output/libqsgepaper.so" "$$stage/libqsgepaper.so"; \
	install -m 0755 scripts/takeover.sh "$$stage/scripts/takeover.sh"; \
	install -m 0644 packaging/receiver-takeover/external.manifest.arm.json \
		"$$stage/external.manifest.json"; \
	install -m 0644 crates/rm-display-receiver/LICENSE-GPL-2.0-only \
		"$$stage/LICENSE-GPL-2.0-only"; \
	version="$$(sed -n 's/^version = "\([^"]*\)"/\1/p' Cargo.toml | head -n 1)"; \
	manifest_version="$$(sed -n 's/.*"version": "\([^"]*\)".*/\1/p' \
		packaging/receiver-takeover/external.manifest.arm.json)"; \
	test -n "$$version" && test "$$version" = "$$manifest_version" || { \
		printf 'manifest version %s does not match Cargo version %s\n' \
			"$$manifest_version" "$$version"; exit 2; }; \
	tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner \
